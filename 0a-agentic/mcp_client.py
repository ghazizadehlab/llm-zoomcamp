...
class MCPClient:
    def __init__(self, server_command: List[str]):
        ...
        self.process = None
        self.request_id = 0
        self.available_tools = {}  # Initialize available_tools as an empty dict
...
def convert_mcp_tool_to_function_format(mcp_tool):
    ...
    # Convert the tool format - use "function" key for OpenAI format
    function_tool = {
        "type": "function",
        "function": {
            "name": name,
    ...
    def get_tools(self):
        if self.tools is None:
            try:
                ...
                print("MCP tools from server:", mcp_tools)
                self.tools = convert_tools_list(mcp_tools)
                print("Converted tools:", self.tools)
                ...
                print("Using hardcoded tools:", self.tools)
        return self.tools

    def function_call(self, tool_call_response):
        # Handle ChatCompletionMessageToolCall object
        if hasattr(tool_call_response, 'function'):
